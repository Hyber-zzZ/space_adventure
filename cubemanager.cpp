...
void CubeManager::createCube(int cubeCount)
{
	for (int i = 0; i < cubeCount; i++) {
		...
		if (i)tempCube.size = rand() % 30/2 + 5;
...
void CubeManager::updateState() {
    for (int i = 0; i < _cubes.length(); i++) {
        ...
        for(int j=0;j<lasers.length();j++){
            ...
            if(laserPos_cube_dis <_cubes[i].size+100&&laserFront_cube_dis< _cubes[i].size*0.6){
		...
		if (dis < _cubes[i].size*0.7 + 3) {
			crashed();
			//m_spacecraft->startShake();
		}
